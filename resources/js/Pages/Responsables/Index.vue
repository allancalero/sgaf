...
const startEdit = (responsable) => {
    editing.value = true;
    editForm.id = responsable.id;
    editForm.nombre = responsable.nombre;
    editForm.id_cargo = responsable.id_cargo ?? '';
    editForm.area_id = responsable.area_id ?? '';
    editForm.estado = responsable.estado || 'ACTIVO';
};

const cancelEdit = () => {
    editing.value = false;
    editForm.reset('id', 'nombre', 'id_cargo', 'area_id', 'estado');
};

const submitCreate = () => {
    createForm.post(route('responsables.store'), {
        onSuccess: () => createForm.reset('nombre', 'id_cargo', 'area_id', 'estado'),
    });
};

const submitEdit = () => {
    editForm.put(route('responsables.update', editForm.id), {
        onSuccess: () => {
            cancelEdit();
        },
    });
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-xl font-semibold leading-tight text-gray-800">Responsables</h2>
                    <p class="text-sm text-gray-500">Asignar responsables por cargo y área.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="mx-auto max-w-6xl space-y-8 sm:px-6 lg:px-8">
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <h3 class="text-lg font-semibold text-gray-800">Crear responsable</h3>
                        <p class="text-sm text-gray-500">Completa los campos requeridos.</p>
                        ...
                        <form class="mt-4 space-y-4" @submit.prevent="submitCreate">
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p v-if="createForm.errors.id_cargo" class="mt-1 text-sm text-red-600">
                                        {{ createForm.errors.id_cargo }}
                                    </p>
                                </div>
                    ...
                    <div class="max-h-[480px] overflow-auto">
                        <table class="min-w-full text-left text-sm text-gray-700">
                            ...
                            <tbody>
                                ...
                                >
                                    <td class="px-4 py-3 text-gray-500">{{ responsable.id }}</td>
                                    <td class="px-4 py-3 font-medium text-gray-900">{{ responsable.nombre }}</td>
                                    <td class="px-4 py-3 text-gray-700">{{ responsable.cargo || '-' }}</td>
                                    <td class="px-4 py-3 text-gray-700">{{ responsable.area || '-' }}</td>
                                    <td class="px-4 py-3">
                                        <span
                                            class="inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold"
                                            :class="responsable.estado === 'ACTIVO'
