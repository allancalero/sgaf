...
const submitCreate = () => {
    createForm.post(route('ubicaciones.store'), {
        onSuccess: () => createForm.reset('nombre'),
    });
};

const submitEdit = () => {
    editForm.put(route('ubicaciones.update', editForm.id), {
        onSuccess: () => {
            cancelEdit();
        },
    });
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-xl font-semibold leading-tight text-gray-800">Ubicaciones</h2>
                    <p class="text-sm text-gray-500">Gestiona ubicaciones físicas o lógicas.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="mx-auto max-w-5xl space-y-8 sm:px-6 lg:px-8">
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <h3 class="text-lg font-semibold text-gray-800">Crear ubicación</h3>
