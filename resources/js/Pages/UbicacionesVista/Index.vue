...
// Forms Áreas
const createAreaForm = useForm({ nombre: '', estado: 'ACTIVO' });
const editAreaForm = useForm({ id: null, nombre: '', estado: 'ACTIVO' });
const editingArea = ref(false);

// Forms Ubicaciones
const createUbicacionForm = useForm({ nombre: '', estado: 'ACTIVO' });
const editUbicacionForm = useForm({ id: null, nombre: '', estado: 'ACTIVO' });
const editingUbicacion = ref(false);

// Funciones Áreas
const submitCreateArea = () => createAreaForm.post(route('areas.store'), { preserveScroll: true, onSuccess: () => createAreaForm.reset() });
const startEditArea = (area) => { editingArea.value = true; editAreaForm.id = area.id; editAreaForm.nombre = area.nombre; editAreaForm.estado = area.estado || 'ACTIVO'; };
const cancelEditArea = () => { editingArea.value = false; editAreaForm.reset(); };
const submitEditArea = () => editAreaForm.put(route('areas.update', editAreaForm.id), { preserveScroll: true, onSuccess: () => { editingArea.value = false; editAreaForm.reset(); } });
const deleteArea = (id) => { if (confirm('¿Estás seguro de eliminar esta área?')) router.delete(route('areas.destroy', id), { preserveScroll: true }); };

// Funciones Ubicaciones
const submitCreateUbicacion = () => createUbicacionForm.post(route('ubicaciones.store'), { preserveScroll: true, onSuccess: () => createUbicacionForm.reset() });
const startEditUbicacion = (ubicacion) => { editingUbicacion.value = true; editUbicacionForm.id = ubicacion.id; editUbicacionForm.nombre = ubicacion.nombre; editUbicacionForm.estado = ubicacion.estado || 'ACTIVO'; };
const cancelEditUbicacion = () => { editingUbicacion.value = false; editUbicacionForm.reset(); };
const submitEditUbicacion = () => editUbicacionForm.put(route('ubicaciones.update', editUbicacionForm.id), { preserveScroll: true, onSuccess: () => { editingUbicacion.value = false; editUbicacionForm.reset(); } });
const deleteUbicacion = (id) => { if (confirm('¿Estás seguro de eliminar esta ubicación?')) router.delete(route('ubicaciones.destroy', id), { preserveScroll: true }); };
</script>
