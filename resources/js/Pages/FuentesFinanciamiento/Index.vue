...
const startEdit = (fuente) => {
    editing.value = true;
    editForm.id = fuente.id;
    editForm.nombre = fuente.nombre;
    editForm.estado = fuente.estado;
};
...
const submitCreate = () => {
    createForm.post(route('fuentes.store'), {
        onSuccess: () => createForm.reset('nombre'),
    });
};

const submitEdit = () => {
    editForm.put(route('fuentes.update', editForm.id), {
        onSuccess: () => {
            cancelEdit();
        },
    });
...
<template>
    <Head title="Fuentes de financiamiento" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-xl font-semibold leading-tight text-gray-800">Fuentes de financiamiento</h2>
                    <p class="text-sm text-gray-500">Registra las fuentes presupuestales.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="mx-auto max-w-5xl space-y-8 sm:px-6 lg:px-8">
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <h3 class="text-lg font-semibold text-gray-800">Crear fuente</h3>
                    ...
                    <div class="max-h-[480px] overflow-auto">
                        <table class="min-w-full text-left text-sm text-gray-700">
                            ...
                            <tbody>
                                ...
                                >
                                    <td class="px-4 py-3 text-gray-500">{{ fuente.id }}</td>
                                    <td class="px-4 py-3 font-medium text-gray-900">{{ fuente.nombre }}</td>
                                    <td class="px-4 py-3">
                                        <span
                                            class="inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold"
                                            :class="fuente.estado === 'ACTIVO'
